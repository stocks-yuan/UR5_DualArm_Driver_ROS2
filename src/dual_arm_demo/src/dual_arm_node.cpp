...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("dual_arm_move_group_node");
...
int main(int argc, char **argv)
{
    ...
    const moveit::core::JointModelGroup *joint_model_group_left =
        left_arm_group.getCurrentState()->getJointModelGroup(LEFT_ARM_GROUP);

    const moveit::core::JointModelGroup *joint_model_group_right =
        right_arm_group.getCurrentState()->getJointModelGroup(RIGHT_ARM_GROUP);

    const moveit::core::JointModelGroup *joint_model_group_dual =
        dual_arm_group.getCurrentState()->getJointModelGroup(DUAL_ARM_GROUP);

    /*************************** Visualization Tools Initialization ****************************/
    ...
    // 打印左右手臂的当前位姿
    geometry_msgs::msg::Pose left_current_pose = dual_arm_group.getCurrentPose("leftArm_tool0").pose;
    ...
    geometry_msgs::msg::Pose right_current_pose = dual_arm_group.getCurrentPose("rightArm_tool0").pose;
    ...
    // 获取左臂当前位姿
    geometry_msgs::msg::Pose start_pose_left = left_arm_group.getCurrentPose("leftArm_tool0").pose;
    // 将当前位姿作为第一个路径点
    ...
    // 获取右臂当前位姿
    geometry_msgs::msg::Pose start_pose_right = right_arm_group.getCurrentPose("rightArm_tool0").pose;
    // 将当前位姿作为第一个路径点
    ...
    // 第一个目标位姿
    target_pose_right.position.x += 0.2;
    target_pose_right.orientation = right_quat;
    waypoints_right.push_back(target_pose_right);

    // 第二个目标位姿
    target_pose_right.position.y += 0.15;
    target_pose_right.orientation = right_quat;
    waypoints_right.push_back(target_pose_right);

    // 第三个目标位姿
    target_pose_right.position.z += 0.2;
    target_pose_right.orientation = right_quat;
    waypoints_right.push_back(target_pose_right);
    ...
    // }
    // 可视化路径
    visual_tools.publishPath(waypoints_left, rvt::LIME_GREEN, rvt::MEDIUM);
    visual_tools.publishPath(waypoints_right, rvt::LIME_GREEN, rvt::MEDIUM);
    ...
    RCLCPP_INFO(LOGGER, "左臂轨迹总时长：%.2f", duration_left);
    RCLCPP_INFO(LOGGER, "右臂轨迹总时长：%.2f", duration_right);
    RCLCPP_INFO(LOGGER, "双臂轨迹总时长：%.2f", max_duration);


    // 确定统一的时间步长
    double time_interval = 0.01; // 时间步长，可以根据需要调整
    size_t num_points = static_cast<size_t>(std::ceil(max_duration / time_interval)) + 1;
    RCLCPP_INFO(LOGGER, "双臂轨迹分割点数：%ld", num_points);
    ...
    // visual_tools.prompt("Press 'next' in the RvizVisualToolsGui window to continue the node");

    // left_arm_group.execute(trajectory_left);
